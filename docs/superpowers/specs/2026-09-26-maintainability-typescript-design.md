...
本文为历史设计记录。白名单部分后来按当时计划实现，现已移除。当前功能见 `docs/plan-and-progress.md`。
...
- 数据与构建产物的流向以架构文档说明：微信消息进入过滤和排队，经 Codex 会话生成回复。账号、回复上下文令牌、会话、轮询游标和媒体文件分别落在哪里。npm 依赖与发布内容由 lockfile、`npm audit` 和 实际 npm tarball 安装检查。
...
- 输出全部生产文件的覆盖率并设置门槛。不能用只统计已加载模块的覆盖率冒充全仓覆盖率。
